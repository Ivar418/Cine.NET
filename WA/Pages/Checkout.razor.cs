...
public partial class Checkout {
    ...
    protected PaymentMethods selectedPaymentMethod = PaymentMethods.Unknown;
    ...
    protected readonly PaymentMethods[] paymentMethods =
        Enum.GetValues<PaymentMethods>()
            .Where(m => m != PaymentMethods.Unknown)
            .ToArray();

    protected readonly Dictionary<PaymentMethods, string> paymentIcons = new() {
        { PaymentMethods.iDEAL, Icons.Material.Filled.AccountBalance },
        { PaymentMethods.Pin, Icons.Material.Filled.PointOfSale },
        { PaymentMethods.CreditCard, Icons.Material.Filled.CreditCard },
        { PaymentMethods.CreditCardOnline, Icons.Material.Filled.Language },
        { PaymentMethods.Reservation, Icons.Material.Filled.Schedule },
        { PaymentMethods.Giftcard, Icons.Material.Filled.CardGiftcard }
    };
    ...
    protected override async Task OnInitializedAsync() {
        try {
            ...
            // Manually parse paymentMethod from query string
            var uri = Nav.ToAbsoluteUri(Nav.Uri);
            var queryString = HttpUtility.ParseQueryString(uri.Query);
            var paymentMethodString = queryString["paymentMethod"];

            if (!string.IsNullOrEmpty(paymentMethodString) && Enum.TryParse(paymentMethodString, true, out PaymentMethods parsedMethod))
            {
    ...
    protected void SelectPaymentMethod(PaymentMethods method) {
        selectedPaymentMethod = method;
    }

    // ── Step 5: build & submit order, then branch ─────────────────────────
    protected async Task ProcessOrderAsync() {
        if (selectedPaymentMethod == PaymentMethods.Unknown) return;
        ...
        if (selectedPaymentMethod == PaymentMethods.Reservation) {
    ...
    private void GoToPaymentMock(CreateOrderResponse order) {
        var chosen = selectedPaymentMethod;
        // Send enough context so PaymentResult can return customer to step 4 in checkout
        var returnUrl = Uri.EscapeDataString(
            $"/checkout/payment-result?orderId={order.OrderId}&showingId={ShowingId}&paymentMethod={Uri.EscapeDataString(chosen.ToString())}");
        ...
        var url = $"/payment-mock" +
                  ...
                  $"&returnUrl={returnUrl}" +
                  $"&ChosenPaymentType={Uri.EscapeDataString(chosen.ToString())}" +
                  $"&reservationId={_confirmedReservation?.Id.ToString() ?? string.Empty}";
